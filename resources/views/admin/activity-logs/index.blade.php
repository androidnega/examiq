@extends('layouts.app', ['header' => __('Activity Logs')])

@section('content')
    <p class="mb-6 text-sm text-slate-500">{{ __('All recorded events (logins, OTP, admin actions, etc.).') }}</p>
    ...
    <x-table>
        ...
        <tbody class="divide-y divide-slate-100">
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-4 py-8 text-center text-sm text-slate-500">{{ __('No activity logged yet.') }}</td>
                </tr>
            @endforelse
        </tbody>
    </x-table>
