@extends('layouts.app', ['header' => __('Edit faculty')])

@section('content')
    <form method="post" action="{{ route('dashboard.faculties.update', $faculty) }}" class="w-full space-y-5">
        @csrf
        @method('PUT')
        <div>
            ...
            <select id="university_id" name="university_id" required class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20">
                @foreach ($universities as $u)
                    <option value="{{ $u->id }}" @selected(old('university_id', $faculty->university_id) === $u->id)>{{ $u->name }}</option>
                @endforeach
            </select>
            @error('university_id')
        ...
        <div>
            ...
            <input id="name" type="text" name="name" value="{{ old('name', $faculty->name) }}" required class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            @error('name')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
