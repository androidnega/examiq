@extends('layouts.app', ['header' => __('Edit department')])

@section('content')
    <form method="post" action="{{ route('dashboard.departments.update', $department) }}" class="w-full space-y-5">
        @csrf
        @method('PUT')
        <div>
            ...
            <select id="faculty_id" name="faculty_id" required class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20">
                @foreach ($faculties as $f)
                    <option value="{{ $f->id }}" @selected(old('faculty_id', $department->faculty_id) === $f->id)>
        ...
        <div>
            ...
            <input id="name" type="text" name="name" value="{{ old('name', $department->name) }}" required class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            @error('name')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
